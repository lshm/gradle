...
When using http://junit.org/junit5/docs/current/user-guide[JUnit Platform and JUnit Jupiter], we scan for methods annotated with annotations of `org.junit.jupiter.api` package.
...
JUnit/JUnit Platform and TestNG allows sophisticated groupings of test methods. Currently, http://junit.org/junit5/docs/current/user-guide/#writing-tests-tagging-and-filtering[JUnit Platform filtering and tagging] is not supported yet.
